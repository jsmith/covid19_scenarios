...
import { SimulationTimePoint, ModelParams } from './types/Result.types'
...
export function initializePopulation(N: number, numCases: number, t0: number, ages: Record<string, number>) {
...
export function evolve(pop: SimulationTimePoint, P: ModelParams, sample: (x: number) => number): SimulationTimePoint {
  const sum = (dict: Record<string, number>) => {
